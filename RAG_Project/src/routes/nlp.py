...
async def answer_rag(request: Request, project_id: str, search_request: SearchRequest):
    ...
    # Convert chat_history to serializable format
    serializable_chat_history = []
    if chat_history:
        for item in chat_history:
            if hasattr(item, 'dict'):
                # If it's a Pydantic model with dict() method
                serializable_chat_history.append(item.dict())
            elif hasattr(item, '__dict__'):
                # If it's a regular object with attributes
                serializable_chat_history.append(item.__dict__)
            else:
                # If it's already serializable
                serializable_chat_history.append(item)
...
@router.get("/index/search/{project_id}")
